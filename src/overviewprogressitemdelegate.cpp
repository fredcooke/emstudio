...
void OverviewProgressItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
	if (index.column() == 1)
	{
		...
		else
		{
			...
			rounded_rect.addRoundRect(option.rect.x(),option.rect.y() + ((option.rect.height() * 0.4) / 2.0),(((double)ivalue / (double)imax) * option.rect.width())-1,(option.rect.height() * 0.6), 20,20);
			...
			else
			{
				...
				painter->setPen(QColor::fromRgb(0,0,0));
				painter->drawText(option.rect.x() + (option.rect.width() / 2.0) - (painter->fontMetrics().width(QString::number((int)(((double)ivalue / (double)imax) * 100.0)) + "%")/2.0),option.rect.y()+ (option.rect.height()/2.0) - (painter->fontMetrics().height()/2.0),option.rect.width(),option.rect.height(),0,QString::number((int)(((double)ivalue / (double)imax) * 100.0)) + "%");
